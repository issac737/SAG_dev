...
from dataflow.core.ai.tokensize import get_mixed_tokenizer
from dataflow.db import SourceEvent, EventEntity, SourceConfig, Article, get_session_factory
...
class RerankRRFSearcher:
    ...
    async def search(
        ...
        try:
            ...
            self.logger.info(
                ...
                f"query='{query}', source_config_ids={source_config_ids[:5]}, threshold={threshold}, top_k={top_k}"
            )
            ...
            # 步骤1-3: 实体匹配 → 事件关联 → 去重（🆕 传入config复用Expand截断配置）
            events, event_to_clues = await self._get_events_from_keys(key_final, source_config_ids, config)
            if not events:
                self.logger.warning(
            ...
            # 步骤3.5: 预截断（基于hop衰减权重预排序，避免向量计算过多事项）
            # 优先保留与 Recall 直接召回实体（hop=0）关联的事项
            max_events_for_embedding = config.rerank.max_events_for_embedding
            if len(events) > max_events_for_embedding:
                # 为每个事项计算预得分（考虑 hop 衰减）
                event_pre_scores = {}
                for event in events:
                    event_id = event.id
                    clues = event_to_clues.get(event_id, [])
                    # 预得分 = Σ(权重 × hop衰减系数)
                    ...
                    for c in clues:
                        ...
                        hop = c.get("hop", 0)
                        hop_decay = 1.0 / (1 + hop)  # hop 越大衰减越多
                        pre_score += weight * hop_decay
                    event_pre_scores[event_id] = pre_score
                
                # 按预得分排序
                events = sorted(events, key=lambda e: event_pre_scores.get(e.id, 0), reverse=True)
                
                # 记录诊断信息
                hop0_count = sum(1 for e in events[:max_events_for_embedding] 
                                for c in event_to_clues.get(e.id, []) if c.get("hop", 0) == 0)
                
                # 截断
                self.logger.info(
                    f"⚠️ 步骤3.5预截断: 事项数({len(events)})超过max_events_for_embedding({max_events_for_embedding})，"
                    f"基于hop衰减权重预排序后截断（保留的事项中包含 {hop0_count} 个hop=0实体关联）"
                )
                events = events[:max_events_for_embedding]
                
                # 更新 event_to_clues（只保留截断后的事项）
                truncated_event_ids = {e.id for e in events}
                event_to_clues = {k: v for k, v in event_to_clues.items() if k in truncated_event_ids}
            ...
            # 步骤5: 阈值过滤
            filtered_events = [e for e in events_with_scores if e.similarity_score >= threshold]
            filtered_count = len(events_with_scores) - len(filtered_events)
            
            if filtered_count > 0:
                scores = [e.similarity_score for e in events_with_scores]
                self.logger.info(
                    f"✅ 步骤5: 阈值过滤完成 (阈值={threshold:.2f})"
                    f"\n  过滤前: {len(events_with_scores)} 个"
                    f"\n  过滤后: {len(filtered_events)} 个"
                    f"\n  过滤掉: {filtered_count} 个"
                    f"\n  最高相似度: {max(scores):.4f}, 最低: {min(scores):.4f}"
                )
            else:
                self.logger.info(
                    f"✅ 步骤5: 所有 {len(events_with_scores)} 个事项都通过阈值 {threshold:.2f}"
                )
            
            if not filtered_events:
                self.logger.warning(f"⚠️ 阈值过滤后没有剩余事项")
                return self._build_response(config, key_final, [], {})
            ...
            self.logger.info(
                ...
                f"(原始={len(events)}, 阈值过滤后={len(filtered_events)}, Top-K={top_k})"
            )
    ...
    async def _get_events_from_keys(
        ...
        config: "SearchConfig" = None  # 🆕 添加 config 参数，复用 Expand 截断配置
        ...
        Args:
            ...
            config: 搜索配置对象（用于读取 Expand 截断配置）
        ...
        # 🆕 从配置读取截断参数（复用 Expand 配置）
        if config:
            MAX_EVENTS_PER_KEY = config.expand.max_events_per_key
            MAX_RECENT_EVENTS = config.expand.max_recent_events
            MAX_RANDOM_EVENTS = config.expand.max_random_events
        else:
            MAX_EVENTS_PER_KEY = 100
            MAX_RECENT_EVENTS = 1000
            MAX_RANDOM_EVENTS = 1000

        async with self.session_factory() as session:
            # 3. 直接通过 EventEntity 查找相关事件（🆕 添加时间排序）
            event_entity_query = (
                select(EventEntity.event_id, EventEntity.entity_id, SourceEvent.created_time)
                ...
                .order_by(SourceEvent.created_time.desc())  # 🆕 按时间倒序
            )

            event_result = await session.execute(event_entity_query)
            event_relations = event_result.fetchall()  # 获取 (event_id, entity_id, created_time) 元组
            original_relation_count = len(event_relations)

            self.logger.info(f"查询到 {original_relation_count} 条 event-entity 关系")

            # 🆕 4. 每个 key 最多取 MAX_EVENTS_PER_KEY 个事项
            key_event_count = defaultdict(int)
            filtered_relations = []
            
            for event_id, entity_id, created_time in event_relations:
                if key_event_count[entity_id] < MAX_EVENTS_PER_KEY:
                    filtered_relations.append((event_id, entity_id))
                    key_event_count[entity_id] += 1
            
            per_key_filtered = original_relation_count - len(filtered_relations)
            if per_key_filtered > 0:
                self.logger.info(
                    f"每key截断: {original_relation_count} → {len(filtered_relations)} "
                    f"(过滤 {per_key_filtered}, 每key最多 {MAX_EVENTS_PER_KEY})"
                )

            # 5. 构建 event_id → clues 映射 + 记录事项顺序（保持时间顺序）
            event_to_clues = {}
            event_order = []  # 🆕 保持时间顺序
            
            for event_id, entity_id in filtered_relations:
            ...
            # 🆕 6. 总数截断（最新 + 随机）
            total_events = len(event_order)
            if total_events > MAX_RECENT_EVENTS:
                recent_event_ids = event_order[:MAX_RECENT_EVENTS]
                older_event_ids = event_order[MAX_RECENT_EVENTS:]
                
                # 从更早的事项中随机抽取
                random_count = min(len(older_event_ids), MAX_RANDOM_EVENTS)
                random_event_ids = random.sample(older_event_ids, random_count) if random_count > 0 else []
                
                # 合并：最新 + 随机
                final_event_ids = set(recent_event_ids) | set(random_event_ids)
                
                # 过滤 event_to_clues
                event_to_clues = {
                    eid: clues for eid, clues in event_to_clues.items() 
                    if eid in final_event_ids
                }
                
                self.logger.info(
                    f"总数截断: {total_events} → {len(final_event_ids)} "
                    f"(最新 {len(recent_event_ids)} + 随机 {random_count})"
                )
            ...
            self.logger.info(f"找到 {len(event_ids)} 个关联事件（截断+去重后）")
    ...
    async def _calculate_embedding_similarity(
        ...
        从 ES 批量获取事项向量，计算与 query 的余弦相似度
        只使用 content_vector（减少 ES 数据传输压力）
        ...
        for i in range(0, len(event_ids), batch_size):
            ...
            for event in events[i:i + batch_size]:
                ...
                # 获取内容向量（不再使用 title_vector）
                content_vector = event_data.get("content_vector")

                # 需要 content_vector
                if content_vector is None:
                    self.logger.warning(f"事项 {event.id} 无 content_vector 数据")
                    missing_vector_count += 1
                    continue
        ...
        # 批量计算相似度（只使用 content_vector）
        self.logger.info(f"开始批量计算 {len(events_with_vectors)} 个事项的相似度...")
        ...
        # 直接计算 content_vector 相似度
        similarities = self._calculate_content_similarity(
        ...
        self.logger.info(
            f"相似度计算完成，耗时: {similarity_time:.4f}秒，"
    ...
    def _calculate_content_similarity(
        ...
        """
        计算内容相似度：query与content批量计算余弦相似度

        Args:
            query_vector: 查询向量
            content_vectors: 内容向量列表

        Returns:
            相似度数组
        """
        try:
            num_items = len(content_vectors)

            # 转换为 numpy 数组以便向量化操作
            query_array = np.array(query_vector, dtype=np.float32)

            # 初始化结果数组
            similarities = np.zeros(num_items, dtype=np.float32)
            ...
            if valid_content_mask.any():
                ...
                similarities[valid_content_mask] = content_sims
        ...
        for rank, event in enumerate(intermediate_events, 1):
            ...
            for entity in source_entities:
                ...
                entity_dict = {
                    ...
                    "hop": entity_info.get("hop", 0) if entity_info else 0  # 🔧 修复：传递 hop 字段
                }
