...
class Tracker:
    ...
    def build_clue(
        ...
        if confidence < 0.0 or confidence > 1000.0:
            logger.warning(
                f"⚠️ [Tracker] 置信度超出范围 [0,1000]: confidence={confidence:.4f}，"
        ...
        # 🐛 调试日志：输出 entity 节点的 hop 信息（仅在 expand 和 recall 阶段）
        if stage in ["recall", "expand"]:
            if from_node.get("type") == "entity":
                logger.debug(
                    f"📍 [Tracker] 创建线索 from entity: "
                    f"name={from_node.get('content', 'N/A')[:20]}, "
                    f"hop={from_node.get('hop', 'N/A')}, "
                    f"stage={stage}, display_level={display_level}"
                )
            if to_node.get("type") == "entity":
                logger.debug(
                    f"📍 [Tracker] 创建线索 to entity: "
                    f"name={to_node.get('content', 'N/A')[:20]}, "
                    f"hop={to_node.get('hop', 'N/A')}, "
                    f"stage={stage}, display_level={display_level}"
                )
