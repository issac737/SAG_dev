...
class ExpandResult:
    ...
    step_average_timings: Dict[str, float] = field(default_factory=dict)  # 每步平均耗时（每跳）
...
class ExpandSearcher:
    ...
    async def search(self, config: SearchConfig, recall_result: Optional[RecallResult] = None) -> ExpandResult:
        ...
        try:
            self.logger.info(
                f"开始实体扩展：source_config_ids={config.source_config_ids[:5]}, query={config.query}, "
            ...
            all_keys_by_jump = {}
            weight_evolution = {}
            step_total_times = {f"step{i}": 0.0 for i in range(1, 6)}
            step_counts = {step: 0 for step in step_total_times}

            # 添加全局去重集合，记录所有发现过的keys
            all_discovered_keys = set(key_final_ids)  # 初始包含Recall的所有keys

            # 🆕 初始化key发现步骤追踪（用于步骤3公式的step衰减）
            key_discovery_steps = {key_id: 1 for key_id in key_final_ids}  # Recall的keys步骤为1
            ...
            for jump in range(1, config.expand.max_hops + 1):
                ...
                event_key_related_2 = await self._step1_keys_to_events(
                    current_key_ids,
                    config.get_source_config_ids(),  # 🔧 传入信息源ID列表
                    config  # 🆕 传入配置以读取截断参数
                )
                step_total_times["step1"] += time.perf_counter() - step_start
                step_counts["step1"] += 1
                ...
                # 2. 向量检索过滤事件
                step_start = time.perf_counter()
                event_query_related, event_similarities = await self._step2_calculate_event_query_similarity(
                    config, event_key_related_2, jump
                )
                step_total_times["step2"] += time.perf_counter() - step_start
                step_counts["step2"] += 1

                filtered_event_ids = [event["event_id"] for event in event_query_related]
                if not filtered_event_ids:
                    self.logger.warning(f"第 {jump} 跳：Step2 没有通过阈值的事件，停止跳跃")
                    break

                all_events_by_jump[jump] = filtered_event_ids

                self.logger.info(
                    f"第 {jump} 跳步骤2：过滤后剩余 {len(filtered_event_ids)} 个事件用于权重计算"
                )

                # 3. 计算event-key权重向量
                step_start = time.perf_counter()
                event_key_2 = await self._step3_calculate_event_key_weights(
                    filtered_event_ids, current_key_ids, current_key_weights,
                    key_discovery_steps  # 🆕 传入key的发现步骤
                )
                step_total_times["step3"] += time.perf_counter() - step_start
                step_counts["step3"] += 1

                self.logger.info(f"第 {jump} 跳步骤3：计算了 {len(event_key_2)} 个events的key权重")

                # 4. 计算event权重向量（简化版，不再乘以query相似度）
                step_start = time.perf_counter()
                event_jump_2 = await self._step4_calculate_event_key_query_weights(
                    event_key_2,
                    config.expand.max_events_per_hop  # Top-N剪枝参数
                )
                step_total_times["step4"] += time.perf_counter() - step_start
                step_counts["step4"] += 1
                ...
                new_key_weights, key_expansion_trace = await self._step5_calculate_key_event_weights(
                    filtered_event_ids, current_key_ids, event_jump_2,
                ...
                step_total_times["step5"] += time.perf_counter() - step_start
                step_counts["step5"] += 1
                ...
                weight_change_abs = abs(current_total_weight - previous_total_weight)
                weight_change = weight_change_abs / previous_total_weight if previous_total_weight > 0 else 1.0
                ...
                jump_result = {
                    ...
                    "events_processed": len(event_jump_2),  # 修改：使用 Step4 处理后的事件数
                ...
                self.logger.info(f"第 {jump} 跳完成：总权重={current_total_weight:.4f}, 权重相对变化={weight_change:.2%}")

                # 检查收敛条件
                if weight_change < config.expand.weight_change_threshold:
                    self.logger.info(f"第 {jump} 跳：权重相对变化 {weight_change:.2%} 小于收敛阈值 {config.expand.weight_change_threshold:.0%}，停止跳跃")
                ...
                for key_id, _ in top_new_keys:
                    ...
                    # 🆕 记录新发现key的步骤（Expand第N跳 = 步骤N+1）
                    key_discovery_steps[key_id] = jump + 1
            ...
            # 汇总最终结果
            step_average_timings = {}
            for step_name, total in step_total_times.items():
                count = step_counts.get(step_name, 0)
                if count > 0:
                    step_average_timings[step_name] = total / count
                ...
                if no_expansion_recall_keys:
                    ...
                    try:
                        ...
                        for key_id in no_expansion_recall_keys:
                            ...
                            # ❌ 不需要构建线索：Recall 阶段已经为所有 recall key 构建了 final 线索
                            # tracker.add_clue(
                            #     stage="expand",
                            #     from_node=Tracker.build_query_node(config),
                            #     to_node=Tracker.build_entity_node(entity_dict),
                        ...
                        self.logger.info(
                            f"✅ [Expand Final] 没有扩展的recall key: {len(no_expansion_recall_keys)} 个（已在 Recall 阶段构建线索，无需重复）"
                        )

                    except Exception as e:
                        self.logger.error(f"⚠️ [Expand Final] 处理没有扩展的recall key失败: {e}", exc_info=True)
            ...
            return ExpandResult(
                ...
                step_average_timings=step_average_timings,
            )
    ...
    async def _step1_keys_to_events(
        ...
        """
        步骤1: 根据keys查找到所有关联的events（限制在指定信息源内）
        用sql找到所有关联事项，得到新的[Event-key-related-2]

        截断策略（从配置读取）：
        - 每个 key 最多关联 max_events_per_key 个事项
        - 按 created_time 倒序，取前 max_recent_events 个最新事项
        - 如果总数超过限制，记录截断时间点，从更早的事项中随机抽取 max_random_events 个
        - 最终最多返回 max_recent_events + max_random_events 个事项

        Args:
            key_ids: 实体ID列表
            source_config_ids: 信息源ID列表（用于过滤）
            config: 搜索配置（可选，用于读取截断参数）
        ...
        # 从配置读取截断参数，如果没有配置则使用默认值
        MAX_EVENTS_PER_KEY = config.expand.max_events_per_key if config else 100
        MAX_RECENT_EVENTS = config.expand.max_recent_events if config else 500
        MAX_RANDOM_EVENTS = config.expand.max_random_events if config else 500
        ...
        self.logger.info(f"  信息源: {source_config_ids[:5]}{'...' if len(source_config_ids) > 5 else ''} (总量={len(source_config_ids)})")
        self.logger.info(f"  截断策略: 每key最多{MAX_EVENTS_PER_KEY}个, 最新{MAX_RECENT_EVENTS}个 + 随机{MAX_RANDOM_EVENTS}个, 最多{MAX_RECENT_EVENTS + MAX_RANDOM_EVENTS}个")

        async with self.session_factory() as session:
            from collections import defaultdict

            # 查询包含这些key的所有event及其created_time，限制在指定信息源内
            query = (
                select(
                ...
                .where(EventEntity.entity_id.in_(key_ids))
                .order_by(EventEntity.created_time.desc())
            )

            result = await session.execute(query)
            all_rows = result.fetchall()

            # 按 key 分组，每个 key 最多取 100 个事项
            key_to_events = defaultdict(list)
            for entity_id, event_id, created_time in all_rows:
                if len(key_to_events[entity_id]) < MAX_EVENTS_PER_KEY:
                    key_to_events[entity_id].append((event_id, created_time))

            # 合并所有事项，去重并按 created_time 排序
            event_time_map = {}  # {event_id: created_time}
            for entity_id, events in key_to_events.items():
                for event_id, created_time in events:
                    # 如果同一个 event 被多个 key 关联，保留最新的时间
                    if event_id not in event_time_map or (created_time and created_time > event_time_map[event_id]):
                        event_time_map[event_id] = created_time

            # 按 created_time 倒序排序（最新的在前）
            sorted_events = sorted(
                event_time_map.items(),
                key=lambda x: x[1] if x[1] else 0,
                reverse=True
            )

            # 取前 MAX_RECENT_EVENTS 个最新事项
            recent_events = sorted_events[:MAX_RECENT_EVENTS]
            recent_event_ids = [event_id for event_id, _ in recent_events]

            # 如果总数超过 MAX_RECENT_EVENTS，记录截断时间并随机抽取
            random_event_ids = []
            cutoff_time = None
            if len(sorted_events) > MAX_RECENT_EVENTS:
                # 记录截断时间点（第 MAX_RECENT_EVENTS 个事项的时间）
                cutoff_time = sorted_events[MAX_RECENT_EVENTS - 1][1]

                # 从更早的事项中随机抽取
                older_events = sorted_events[MAX_RECENT_EVENTS:]
                sample_size = min(len(older_events), MAX_RANDOM_EVENTS)
                random_sample = random.sample(older_events, sample_size)
                random_event_ids = [event_id for event_id, _ in random_sample]

            # 合并结果
            event_ids = recent_event_ids + random_event_ids

            # 统计截断情况
            total_before_key_truncation = len(all_rows)
            total_after_key_truncation = sum(len(events) for events in key_to_events.values())
            unique_events_before_total_truncation = len(event_time_map)

            self.logger.info(f"  📊 截断统计:")
            self.logger.info(f"    • 原始 key-event 关联数: {total_before_key_truncation}")
            self.logger.info(f"    • 每key截断后关联数: {total_after_key_truncation}")
            self.logger.info(f"    • 去重后事项数: {unique_events_before_total_truncation}")
            self.logger.info(f"    • 最新事项数: {len(recent_event_ids)}")
            if cutoff_time:
                self.logger.info(f"    • 截断时间点: {cutoff_time}")
                self.logger.info(f"    • 随机抽取数: {len(random_event_ids)}")
            self.logger.info(f"    • 最终事项数: {len(event_ids)}")

            # 🔍 诊断：查询这些事项的 source_type 分布
            if event_ids:
                from collections import Counter
                type_query = (
                    select(SourceEvent.source_type, SourceEvent.source_config_id)
                    .where(SourceEvent.id.in_(event_ids[:100]))  # 取前100个样本
                )
                type_result = await session.execute(type_query)
                type_rows = type_result.fetchall()

                type_counter = Counter(row[0] for row in type_rows)
                source_counter = Counter(row[1] for row in type_rows)

                self.logger.info(f"  🔍 事项类型分布 (样本{len(type_rows)}个): {dict(type_counter)}")
                self.logger.info(f"  🔍 信息源分布 (样本{len(type_rows)}个): {dict(source_counter)}")

        # 调试：输出关联的event_ids
        self.logger.info(f"步骤1完成:")
        self.logger.info(f"  • 输入keys数量: {len(key_ids)}")
        self.logger.info(f"  • Keys关联events数量: {len(event_ids)} (最新+随机抽取)")

        # =====================================================
        # 🆕 并集操作：合并高质量事项（新策略步骤1召回的）
        # =====================================================
        keys_event_count = len(event_ids)
        high_quality_count = 0
        
        if config and hasattr(config, 'high_quality_event_ids') and config.high_quality_event_ids:
            high_quality_event_ids = config.high_quality_event_ids
            high_quality_count = len(high_quality_event_ids)
            
            # 并集：Keys→Events ∪ Query→Events（高质量）
            event_ids_set = set(event_ids)
            original_count = len(event_ids_set)
            
            # 加入高质量事项
            event_ids_set |= high_quality_event_ids
            
            # 转回列表
            event_ids = list(event_ids_set)
            added_count = len(event_ids) - original_count
            
            self.logger.info(
                f"  🔀 并集合并: Keys→Events({keys_event_count}) ∪ Query→Events({high_quality_count}) "
                f"= {len(event_ids)} (新增 {added_count} 个高质量事项)"
            )
        else:
            self.logger.info(f"  ⚠️ 无高质量事项可合并（high_quality_event_ids 为空）")

        # Step1 总量限制
        max_events_limit = config.expand.step1_events_limit if config else 5000
        if len(event_ids) > max_events_limit:
            self.logger.info(
                f"  ✂️  Step1事项触达上限: {len(event_ids)} -> {max_events_limit} (limit={max_events_limit})"
            )
            event_ids = event_ids[:max_events_limit]

        self.logger.info(f"  • 最终events数量: {len(event_ids)}")
    ...
    async def _step2_calculate_event_query_similarity(
        self, config: SearchConfig, event_ids: List[str], jump: int = 1
    ) -> Tuple[List[Dict[str, Any]], Dict[str, float]]:
        """
        步骤2: 使用向量检索过滤事件（限定在 Step1 给定的 event_ids 内）
        仅保留与 query 相似度达到阈值的 Top-K 事件
        """
        if not event_ids:
            return [], {}

        self.logger.info(
            f"步骤2: ES向量检索过滤 {len(event_ids)} 个候选事件 "
            f"(top_k={config.expand.max_events_per_key}, threshold={config.expand.event_similarity_threshold})"
        )
        ...
        # 限制返回数量
        top_k = min(config.expand.max_events_per_key, len(event_ids))
        threshold = config.expand.event_similarity_threshold

        # 仅在给定的 event_ids 中进行向量检索
        try:
            vector_results = await self.event_repo.search_similar_by_content(
            ...
            self.logger.error(f"步骤2向量检索失败: {e}")
            return [], {}

        event_query_related: List[Dict[str, Any]] = []
        event_similarities: Dict[str, float] = {}
        ...
        for doc in vector_results:
            ...
            similarity = float(doc.get("_score", 0.0))
            ...
            event_info = {
                ...
                "match_type": "vector_filtered_content",
            }
            event_query_related.append(event_info)
            event_similarities[event_id] = similarity

        self.logger.info(f"步骤2完成: Top{top_k} 中有 {len(event_query_related)} 个事件满足阈值 >= {threshold}")
        if event_query_related:
            similarities = list(event_similarities.values())
            self.logger.info(
                f"  • 相似度范围: {min(similarities):.4f} - {max(similarities):.4f}"
            )
    ...
    async def _step3_calculate_event_key_weights(
        ...
        key_ids: List[str],
        key_weights: Dict[str, float],
        key_steps: Dict[str, int],  # 🆕 新增参数：key的发现步骤
    ) -> Dict[str, float]:
        """
        步骤3: 计算Event-key-related-2权重向量
        公式: W(e_j) = ln(1 + Σ (W_K(k_i) * ln(1 + count(k_i, e_j)) / step(k_i)))

        其中:
        - W_K(k_i): key的权重
        - count(k_i, e_j): key在event文本中出现的次数
        - step(k_i): key被发现的步骤（用于衰减）
        ...
        try:
            async with self.session_factory() as session:
                ...
                # 2. 批量查询 event 内容（用于计算 count）
                event_query = (
                    select(SourceEvent.id, SourceEvent.title, SourceEvent.content)
                    .where(SourceEvent.id.in_(event_ids))
                )
                event_result = await session.execute(event_query)
                event_contents = {
                    row.id: (row.title or "") + " " + (row.content or "")
                    for row in event_result.fetchall()
                }

                # 3. 批量查询 entity 名称（用于计算 count）
                entity_query = select(Entity.id, Entity.name).where(Entity.id.in_(key_ids))
                entity_result = await session.execute(entity_query)
                entity_names = {row.id: row.name for row in entity_result.fetchall()}

                # 4. 按公式计算每个event的权重
                for event_id in event_ids:
                    event_keys = event_to_keys.get(event_id, [])
                    event_text = event_contents.get(event_id, "").lower()

                    # Σ (W_K(k_i) * ln(1 + count) / step(k_i))
                    inner_sum = 0.0
                    for key_id in event_keys:
                        w_k = key_weights.get(key_id, 0.0)
                        step = key_steps.get(key_id, 1)  # 默认步骤1

                        # 🆕 动态计算 count：entity name 在 event 文本中出现次数
                        entity_name = entity_names.get(key_id, "")
                        count = event_text.count(entity_name.lower()) if entity_name else 1
                        count = max(1, count)  # 至少为1

                        # 应用公式: W_K * ln(1 + count) / step
                        inner_sum += (w_k * math.log(1 + count)) / step

                    # 外层 ln(1 + ...)
                    event_key_weights[event_id] = math.log(1 + inner_sum)
    ...
    async def _step4_calculate_event_key_query_weights(
        ...
        """
        步骤4: 计算event权重向量 + Top-N剪枝（简化版）

        简化说明：
        - 原版本将 event-key 权重乘以 query-event 相似度
        - 现在直接使用 event-key 权重，不再考虑 query 相似度
        - 保留 Top-N 剪枝和归一化逻辑
        """
        if not event_key_weights:
            return {}
        ...
        # 直接使用 event_key_weights 作为结果
        event_weights = event_key_weights.copy()

        # 统计非零权重事件
        non_zero_count = sum(1 for w in event_weights.values() if w > 0)
        zero_weight_events = len(event_weights) - non_zero_count

        # 输出统计信息
        self.logger.info(f"步骤4权重计算统计:")
        self.logger.info(f"  • 总事件数: {len(event_weights)}")
        self.logger.info(f"  • 非零权重事件: {non_zero_count}")
        self.logger.info(f"  • 权重为0的事件: {zero_weight_events}")

        # 如果权重为0的事件过多，发出警告
        if zero_weight_events > len(event_weights) * 0.8:
            self.logger.warning(f"⚠️ 步骤4中有 {zero_weight_events}/{len(event_weights)} 个事件权重为0，可能影响搜索效果")

        # Top-N 剪枝：选取权重最高的 max_events 个事件
        if len(event_weights) > max_events:
            sorted_events = sorted(
                event_weights.items(),
                key=lambda x: x[1],
                reverse=True
            ...
            pruned_count = len(event_weights) - max_events
            event_weights = dict(top_events)

            self.logger.info(
                f"步骤4 Top-N剪枝: {len(sorted_events)} -> {max_events} events (剪枝{pruned_count}个)"
            )

        # 权重归一化和容错处理
        if event_weights:
            # 过滤掉权重为0的事件
            non_zero_weights = {k: v for k, v in event_weights.items() if v > 0}
    ...
    async def _step5_calculate_key_event_weights(
        ...
        类型过滤：优先使用 focus_types（白名单），回退 exclude_types（黑名单）
        ...
        if focus_types:
            self.logger.info(f"  🎯 聚焦实体类型(白名单): {focus_types}")
        elif exclude_types:
            self.logger.info(f"  🚫 排除实体类型(黑名单): {exclude_types}")
        ...
        try:
            async with self.session_factory() as session:
                ...
                if focus_types:
                    # tags 作为兜底（应对分类偏差）
                    effective_types = list(set(focus_types) | {"tags"})
                    ...
                    entity_event_query = (
                        ...
                        .where(EventEntity.event_id.in_(event_ids))
                        .where(Entity.type.in_(effective_types))
                    )
                elif exclude_types:
                    # 黑名单：排除指定类型的实体
                    entity_event_query = (
                        ...
                        .join(Entity, EventEntity.entity_id == Entity.id)
                        .where(EventEntity.event_id.in_(event_ids))
                        .where(~Entity.type.in_(exclude_types))
                    )
                else:
                    # 不需要过滤时，保持原有逻辑
                    entity_event_query = (
                ...
                for entity_id in all_entity_ids:
                    ...
                    # 🆕 应用新公式: (Σ W / Count_i) * min(2, ln(1 + Count_i))
                    # Σ W_event-key-query(e_j)
                    sum_weight = sum(
                        event_weights.get(event_id, 0.0) for event_id in entity_events
                    )

                    # Count_i = 该key关联的event数量
                    count_i = len(entity_events)

                    if count_i > 0:
                        # 应用完整公式: (Σ W / Count_i) * min(2, ln(1 + Count_i))
                        avg_weight = sum_weight / count_i
                        frequency_factor = min(2.0, math.log(1 + count_i))
                        total_weight = avg_weight * frequency_factor
                    else:
                        total_weight = 0.0
