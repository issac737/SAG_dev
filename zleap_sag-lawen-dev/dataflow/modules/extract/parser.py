...
class EntityValueParser:
    ...
    def _parse_datetime(
        ...
        支持格式：
        - ISO 格式：2024-01-15, 2024-01-15T14:30:00, 2024-01-15 14:30:00
        - 中文格式：2024年1月15日, 2025年11月28日 05:52:25
        - 描述性：今天, 昨天, 上个月
        """
        # 🔧 改进：更谨慎的中文字符替换
        text_clean = text.replace('年', '-').replace('月', '-').replace('日', '')
        text_clean = re.sub(r'-+', '-', text_clean)  # 合并多个连续的减号
        text_clean = text_clean.strip('-')  # 去除首尾的减号

        # 🆕 模式1：完整日期时间 YYYY-MM-DD HH:MM:SS 或 YYYY-MM-DDTHH:MM:SS
        # 支持空格或T作为分隔符
        ...
        # 🆕 模式2：日期时间（仅到分钟） YYYY-MM-DD HH:MM 或 YYYY-MM-DDTHH:MM
        ...
        # 模式4：YYYY-MM（如 "2024-01" 或 "2024年1月"）
        ...
        # 🔧 模式5：仅年份 YYYY（增强：综合判断避免误判金额）
        # 🆕 更完整的货币相关模式检查
        money_patterns = [
            r'元', r'美元', r'USD', r'\$', r'¥', r'万元', r'亿元',  # 货币单位
            r'块', r'角', r'分', r'块钱', r'毛',  # 口语化
            r'EUR', r'£', r'RMB', r'CNY', r'HKD',  # 其他货币
            r'/月', r'/年', r'/天', r'/周', r'每月', r'每年',  # 周期
            r'起', r'左右', r'约',  # 价格修饰词
        ]
        has_money_indicator = any(re.search(pattern, text, re.IGNORECASE) for pattern in money_patterns)
        ...
        if match and not has_money_indicator:
            ...
            # 🔧 更严格的年份范围：拒绝明显是金额的4位数（如2000-9999中过大的数值）
            # 策略：年份通常在1970-2099范围内；超出此范围且没有其他时间上下文，可能是金额
            if 1970 <= year <= 2099:
                try:
    ...
    def _parse_compact_datetime(self, text: str) -> Optional[Dict[str, Any]]:
        ...
        - 🆕 YYYYMMDDHHmmss (14位)：20230117143025 → 2023-01-17 14:30:25
        - 🆕 YYYYMMDDHHmm (12位)：202301171430 → 2023-01-17 14:30:00
        ...
        Returns:
            解析结果或 None（日期无效时）
        """
        # 🆕 模式1：紧凑日期时间格式 YYYYMMDDHHmmss（14位）
        pattern_compact_full_dt = r'^(\d{14})$'
        match = re.match(pattern_compact_full_dt, text)
        if match:
            try:
                year = int(text[0:4])
                month = int(text[4:6])
        ...
        # 🆕 模式2：紧凑日期时间格式（到分钟） YYYYMMDDHHmm（12位）
        pattern_compact_dt_min = r'^(\d{12})$'
        match = re.match(pattern_compact_dt_min, text)
        if match:
            try:
                year = int(text[0:4])
                month = int(text[4:6])
