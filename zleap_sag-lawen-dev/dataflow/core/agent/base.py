...
class BaseAgent:
    ...
    def __init__(
        ...
        Args:
            timezone: 时区（可选，默认使用配置文件中的配置）
            language: 语言（可选，默认使用配置文件中的配置）
            database: 初始数据库分区列表（可选，默认使用配置文件中的配置）
            memory: 初始记忆分区列表（可选，默认使用配置文件中的配置）
            todo: 初始待办任务列表（可选，默认使用配置文件中的配置）
            output: 输出配置覆盖（可选）
            ...
            scenario: LLM场景标识（默认 'chat'）
                ...
                - 'summary': 总结
                - 'general': 通用
            agent_config: Agent配置字典（可选）
                - 如果传入：使用该配置（子类可传入自定义配置如 extractor.json）
                - 如果不传：使用默认的 agent.json
        ...
        Example:
            ...
            # 子类传入自定义配置
            agent = BaseAgent(
                agent_config=custom_config  # 使用自定义配置文件
            )
        """
        # 1. 加载基本配置
        ...
        if agent_config:
            ...
            logger.info(f"使用自定义Agent配置: {agent_config.get('name', 'Unknown')}")
        else:
            # 使用默认的 agent.json
        ...
        config_data = self.agent_config["config"]
        self.timezone = timezone or role.get("timezone", "Asia/Shanghai")
        self.language = language or role.get("language", "zh-CN")

        # 3. 初始化数据（优先使用参数，否则使用配置文件中的默认值）
        self.database: List[Dict[str, Any]] = (
            database if database is not None else config_data.get("database", [])
        )
        self.memory: List[Dict[str, Any]] = (
            memory if memory is not None else config_data.get("memory", [])
        )
        self.todo: List[Dict[str, Any]] = todo if todo is not None else config_data.get("todo", [])
    ...
    async def _get_llm_client(self) -> BaseLLMClient:
        ...
        if self._llm_client is None:
            ...
            self._llm_client = await create_llm_client(
                scenario=self.scenario, model_config=self.model_config
            )
    ...
    def _build_system_prompt(
        self, output_overrides: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """构建系统提示词（总是注入当前时间）"""
        # 总是注入当前时间
    ...
    def _find_partition(self, partitions: List[Dict], data_type: str) -> Optional[Dict]:
        """
        查找分区
