 ...
 * 搜索响应类型定义
 *
 * 对应后端 dataflow/modules/search 模块的返回数据结构
 */
...
/**
 * 路径节点项（用于 lines 和 event_lines）
 *
 * 每个路径节点可以是 query、entity 或 event 之一
 */
...
export interface SearchResponse {
   ...
   * 每个事项的最短路径
   *
   * 格式：{ "event-id": [{"query": ...}, {"entity": ...}, {"event": ...}] }
   *
   * 用于前端精简模式图谱展示
   */
   ...
   * 按跳数分组的实体统计
   *
   * 格式：{ "0": [entity1, entity2, ...], "1": [...], "2": [...] }
   * - "0": Recall 召回的实体
   * - "1": Expand 第1跳实体
   * - "2": Expand 第2跳实体
   */
  entitys?: Record<string, any[]>;
   ...
   * }
   *
   * 用于前端展示所有推理路径（仅 Rerank 阶段的事项）
   */
  rerank_lines?: Record<string, PathItem[][]>;
}
