...
import type { Clue, GraphData, GraphNode, GraphLine, Node, PathItem, SearchResponse } from "@/types/search-response";
 ...
 * - 节点/连线点击交互
 */
export function CluesGraph({ clues, height = 600, searchResponse }: CluesGraphProps) {
  ...
  const graphData = useMemo(() => {
    // 🔍 调试：输出 searchResponse 的内容
    console.log('=== Graph Data Debug ===');
    console.log('isSimplifiedMode:', isSimplifiedMode);
    console.log('selectedStages:', selectedStages);
    console.log('searchResponse exists:', !!searchResponse);
    console.log('searchResponse?.rerank_lines exists:', !!searchResponse?.rerank_lines);
    console.log('searchResponse?.rerank_lines keys:', searchResponse?.rerank_lines ? Object.keys(searchResponse.rerank_lines).length : 'N/A');

    // 🆕 精简模式 + Rerank 阶段：使用 rerank_lines（所有路径）
    if (isSimplifiedMode && selectedStages.includes('rerank') && searchResponse?.rerank_lines) {
      console.log('✅ Using rerank_lines (Rerank stage in simplified mode)');
      const data = convertRerankLinesToGraphData(searchResponse.rerank_lines);
      console.log('Graph data (from rerank_lines):', data);
      console.log('Nodes count:', data.nodes.length);
      console.log('Lines count:', data.lines.length);
      console.log('Events count:', Object.keys(searchResponse.rerank_lines).length);
      return data;
    }

    // 其他情况：使用 filterCluesByDisplayMode（包括精简模式的 Recall/Expand）
    console.log('📋 Using clues with filterCluesByDisplayMode');
    ...
    console.log('Graph data (from clues):', data);
    console.log('Nodes count:', data.nodes.length);
    console.log('Lines count:', data.lines.length);
    return data;
  }, [filteredClues, isSimplifiedMode, selectedStages, searchResponse]);
 ...
 * }
 */
function convertRerankLinesToGraphData(rerankLines: Record<string, PathItem[][]>): GraphData {
  const nodesMap = new Map<string, GraphNode>();
  const linesMap = new Map<string, GraphLine>();  // 用于去重连线

  // 遍历所有事项的所有路径
  Object.entries(rerankLines).forEach(([eventId, paths]) => {
    paths.forEach((pathItems) => {
      // 遍历路径中的每一步
      for (let i = 0; i < pathItems.length; i++) {
        const item = pathItems[i];
        const node = item.query || item.entity || item.event;

        if (!node) continue;

        // 添加节点
        if (!nodesMap.has(node.id)) {
          // 🆕 只有 Rerank 阶段的 final 事项才使用紫色高亮
          let typeColor = getTypeColor(node.type);
          if (node.type === 'event' && node.stage === 'rerank') {
            typeColor = '#9c27b0';  // 紫色高亮
          }

          nodesMap.set(node.id, {
            id: node.id,
            text: '',
            html: generateNodeHtml(node, typeColor),
            type: node.type,
            color: 'transparent',
            fontColor: "#ffffff",
            fontSize: 12,
            width: node.type === 'event' ? 210 : 180,
            height: node.type === 'event' ? 90 : 85,
            borderWidth: 0,
            borderColor: 'transparent',
            data: node,
          });
        ...
        // 添加连线（如果不是第一个节点）
        if (i > 0) {
          const prevItem = pathItems[i - 1];
          const prevNode = prevItem.query || prevItem.entity || prevItem.event;

          if (prevNode) {
            // 去重：同一个 from → to 只添加一条线
            const lineKey = `${prevNode.id}->${node.id}`;

            if (!linesMap.has(lineKey)) {
              // 根据节点类型判断阶段和样式
              let stage = '';
              let stageColor = '#90caf9';
              let lineWidth = 2;

              if (prevNode.type === 'query' && node.type === 'entity') {
                stage = '召回';
                stageColor = '#1890ff';
                lineWidth = 2.5;
              } else if (prevNode.type === 'entity' && node.type === 'entity') {
                stage = '拓展';
                stageColor = '#52c41a';
                lineWidth = 2;
              } else if (prevNode.type === 'entity' && node.type === 'event') {
                stage = '重排';
                stageColor = '#fa8c16';
                lineWidth = 3;
              } else if (prevNode.type === 'query' && node.type === 'event') {
                stage = '召回';
                stageColor = '#1890ff';
                lineWidth = 2.5;
              }
  ...
  return {
    rootId,
    nodes: Array.from(nodesMap.values()),
    lines: Array.from(linesMap.values()),  // 使用去重后的连线
  };
}
