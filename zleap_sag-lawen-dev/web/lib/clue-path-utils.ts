...
export function findAllCompletePaths(
  ...
  function dfs(
    ...
    // 停止条件1：到达 origin query 节点（真正的起点）
    // 🆕 修复：只有 category="origin" 的 query 才是真正的起点
    // category="rewrite" 的 query 需要继续往上追溯到 origin query
    if (currentNode.type === 'query' && currentNode.category === 'origin') {
      allPaths.push([...currentPath]);
      return;
    }

    // 🆕 如果是 rewrite query，继续往上追溯（不停止），这样可以包含 prepare 阶段的 query 重写线索
